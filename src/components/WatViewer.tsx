...
interface WatViewerProps {
	content: string;
	hasContent: boolean;
}

export const WatViewer: Component<WatViewerProps> = (props) => {
		...
		<div class="flex flex-col h-full">
				...
				<Show when={props.hasContent}>
					<div class="flex gap-2">
						<button
							...
							class="px-3 py-1 text-sm bg-secondaryContainer text-onSecondaryContainer rounded-full hover:bg-secondaryContainer/80 transition-colors font-medium"
						>
							Copy
						</button>
						<button
							onClick={downloadFile}
							class="px-3 py-1 text-sm bg-primary text-onPrimary rounded-full hover:bg-primary/90 transition-colors font-medium"
						>
							Download .wat
						</button>
					</div>
			...
			<div class="flex-grow bg-surfaceVariant/30 rounded-lg p-0 relative overflow-hidden border border-outline/10">
				<Show
					...
					fallback={
						<div class="flex items-center justify-center h-full text-outline italic p-4">
